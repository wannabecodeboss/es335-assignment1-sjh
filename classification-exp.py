...
# FIX: Convert predictions to pandas Series and reset indices
...
# Visualize the tree - FIX: Remove filename parameter
    ...
    for train_idx, test_idx in outer_kf.split(X):
        ...
        for depth in depth_range:
            ...
            for inner_train_idx, inner_val_idx in inner_kf.split(X_outer_train):
                ...
                # Validate - FIX: Reset indices for metric calculation
                y_inner_pred = dt_inner.predict(X_inner_val)
                y_inner_pred = pd.Series(y_inner_pred).reset_index(drop=True)
                y_inner_val = y_inner_val.reset_index(drop=True)
        ...
        # Evaluate on outer test set - FIX: Reset indices for metric calculation
        y_outer_pred = dt_final.predict(X_outer_test)
        y_outer_pred = pd.Series(y_outer_pred).reset_index(drop=True)
...
for depth in depth_range:
    ...
    for train_idx, val_idx in kf.split(X_df):
        ...
        # FIX: Reset indices for metric calculation
        y_pred_cv = dt_cv.predict(X_val_cv)
        y_pred_cv = pd.Series(y_pred_cv).reset_index(drop=True)
        y_val_cv = y_val_cv.reset_index(drop=True)
...
# Plot final optimized tree - FIX: Remove filename parameter
final_dt.plot()
